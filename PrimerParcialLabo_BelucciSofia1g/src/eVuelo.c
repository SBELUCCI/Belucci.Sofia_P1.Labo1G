...
int altaVuelo(int *pIndice, eVuelo vuelos[], int tamVuelos, eAvion aviones[],
	...
	if (pIndice != NULL && vuelos != NULL && tamVuelos > 0) {
		...
		if (indice == OCUPADO) {
			...
			utn_getNumeroInt(&auxVuelo.idDestino,
					...
					"ERROR: ingrese un ID de destino válido ", 0, 9999999, 100);
			...
			while (!validarIdDestino(auxVuelo.idDestino, destinos, tamDestinos)) {
				...
				utn_getNumeroInt(&auxVuelo.idDestino,
						...
						"ERROR: ingrese un ID de destino válido ", 0, 9999999,
						100);
			...
			utn_getNumeroInt(&auxVuelo.fechaVuelo.dia,
					"Ingrese el día deseado del vuelo:  ",
			...
			utn_getNumeroInt(&auxVuelo.fechaVuelo.anio,
					"Ingrese el año deseado del vuelo:  ",
					"Error: ingrese un año valido", 1, 2030, 100);
